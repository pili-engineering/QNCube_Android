...
import com.niucube.lrcview.bean.LrcEntryData;

import java.util.List;

import com.niucube.lrcview.bean.LrcData;

import io.niucube.lrcview.R;
...
public class LrcView extends View {
    ...
    private void updateTime2(long time) {
    ...
    private int startAutoUpDataLoopTime = 0;
    private Runnable runnable= new Runnable(){

        @Override
        public void run() {
            startAutoUpDataLoopTime++;
            if(startAutoUpDataLoopTime>=1000/startAutoUpDataSpan){
                return;
            }
            mCurrentTime +=startAutoUpDataSpan;
            handler.postDelayed(runnable,startAutoUpDataSpan);
            updateTime2(mCurrentTime);

        }
    };

    private void stopAutoUpData(){
        startAutoUpDataLoopTime=0;
        handler.removeCallbacks(runnable);
    }
    private int startAutoUpDataSpan = 25;
    private void startAutoUpData(){
         stopAutoUpData();
        handler.postDelayed(runnable,startAutoUpDataSpan);
    }
    ...
    private boolean kalaOk = true;

    public void setIsKalaOk(boolean isKalaOk){
        this.kalaOk = isKalaOk;
    }
    ...
    private void drawHighLight(  LrcEntryData cur) {
        if (curLrcEntry == null) {
            return;
        }
        ...
        Rect[] drawRects;
        if(kalaOk){
            drawRects = curLrcEntry.getDrawRectByTime(mCurrentTime);
        }else {
            drawRects = curLrcEntry.getDrawRectByTime(cur.tones.get(cur.tones.size()-1).end);
        }
