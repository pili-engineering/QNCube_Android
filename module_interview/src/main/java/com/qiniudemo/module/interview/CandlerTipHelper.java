...
public class CandlerTipHelper {
     ...
     * 删除日历事件
     */
    public static void deleteCalendarEvent(Context context, String title, long reminderTime) {
        if (context == null) {
            return;
        }
        ...
        try {
            ...
            if (eventCursor.getCount() > 0) {
                ...
                for (eventCursor.moveToFirst(); !eventCursor.isAfterLast(); eventCursor.moveToNext()) {
                    ...
                    @SuppressLint("Range") long time = eventCursor.getLong(eventCursor.getColumnIndex(CalendarContract.Events.DTSTART));
                    if (!TextUtils.isEmpty(title) && title.equals(eventTitle) && time == reminderTime) {
