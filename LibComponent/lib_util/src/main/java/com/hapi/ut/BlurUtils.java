...
public class BlurUtils {
     ...
     * @return
     */
    public static Bitmap blurWithLocal(Bitmap image) {
        try {
            int width = Math.round(image.getWidth() * BITMAP_SCALE);
            int height = Math.round(image.getHeight() * BITMAP_SCALE);

            Bitmap overlay = Bitmap.createScaledBitmap(image, (int) (width),
                    (int) (height), false);

            overlay = BlurUtils.doBlur(overlay, (int) BLUR_RADIUS, true);
